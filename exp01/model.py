...
class ResNet(nn.Module):
    ...
    def __init__(self, residual, num_residuals, num_classes=10, include_top=True):
        ...
        self.conv1 = nn.Conv2d(3, self.out_channel, kernel_size=7, stride=2, padding=3,
                               bias=False)
        # 3表示输入特征图像的RGB通道数为3，即图片数据的输入通道为3
        # 1表示输入特征图像的灰度通道数为1，即图片数据的输入通道为1
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                # kaiming_normal_：使用Kaiming正态分布初始化卷积层的权重
                # mode='fan_out'：基于输出通道数进行缩放
                # nonlinearity='relu'：专门为ReLU激活函数优化
